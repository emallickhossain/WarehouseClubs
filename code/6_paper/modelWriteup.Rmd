...
## Multinomial Logit
I start by estimating a basic multinomial logit model that represents a consumer's purchase decision. Each product is characterized by its ply, package size, standardized rolls per package, unit price, and brand.^[I draw a distinction between package size and standardized rolls per package to better isolate how long a particular package may last. A 4-roll package is likely to take up about the same amount of space regardless of ply or sheet count, but may last for longer or shorter durations depending on those characteristics. Going back to at least 1915, this has been a source of product obfuscation. See 1915 Scott ad included in Appendix Figure \ref{fig:Scott_ad_1915}] I aggregate packages into *small* (1-6 rolls), *medium* (7-12 rolls), and *large* (12+ rolls).^[While there are a range of sizes offered, most packages are 4-, 6-, 12-, or 24-roll packages. This categorization captures the main package size variation while also incorporating other, less popular sizes. It also captures the main focus of package size, namely it serving as a proxy for storage and transportation costs.] I estimate this model where the consumer can choose from one of the top 5 brands (Angel Soft, Charmin, Cottonelle, Quilted Northern, and Scott) or a private-label^[Private label also includes the long tail of other, less popular brands.] and 3 different sizes (small, medium, or large). This generates a total of 18 brand-sizes. 
...
Households choose the product that maximizes their utility, which I represent as the following (time and market subscripts are suppressed for clarity; notation follows @cohen2008).

\begin{equation}
U_{ij} = \phi pkgSize_j + \gamma size_j + \alpha p_j + \mu_{ij}
\end{equation}

where $U_{ij}$ represents the utility that household $i$ gets from product $j$. $pkgSize$ is a categorical variable for small, medium, or large packages; $size$ is the standardized rolls in a package; and $p$ is the per-standardized-roll cost of package $j$. Letting $\mu_{ij} = \epsilon_{ij}$ where $\epsilon$ is drawn from a Type I Extreme Value distribution gives us the standard multinomial logit model. The choice probabilities of can be computed from the following expression:

\begin{equation}
P_j = \frac{exp(\delta_j)}{\sum_{h = 1}^J exp(\delta_h)},
\end{equation}

where $\delta_j = \phi pkgSize_j + \gamma size_j + \alpha p_j$, which can be thought of as the average population utility for product $j$. The parameters of the utility function can be estimated using MLE and the estimated coefficients represent the average population preferences for each product characteristic.
...
## Conditional Logit
The above model is quite restrictive along a variety of dimensions. Primarily, it does not incorporate any observed heterogeneity that is available in the data (any heterogeneity is only incorporated through $\epsilon$). Furthermore, it generates restrictive substitution patterns as a result of the Independence of Irrelevant Alternatives (IIA) property, which means that when the price of one item increases, consumers will substitute proportionally to other items, which results in them substituting to the most popular items instead of substituting to the items most similar to the one they were considering. I address the first concern by directly incorporating demographic characteristics. In particular, it is intuititive that price sensitivity may be related to a household's income. Second, a household's sensitivity to package size may be related to the type of home they have. Finally, the preference for the standardized amount may be related to a household's consumption rate. In order to incorporate each of these features, I interact the relevant product characteristic with the corresponding household characteristic and re-estimate the model.

\input{./tables/mnlBostonCond.tex}

## Random Coefficients
While the conditional logit model incorporates observed household heterogeneity, it still maintains the restrictive substitution patterns that result from the IIA property. In order to allow for flexible substitution patterns and unobserved household heterogeneity, I estimate a random coefficients model, while maintaining the observed household characteristics that I included in the conditional logit model. The random coefficients model is represented below (suppressing the )
...
|Restrict to top brand/size combos |   826,630|  74,642|
|Single packages                   |   811,819|  74,446|
...
\begin{figure}
\centering
\includegraphics[height=0.9\textheight]{../5_figures/Scott_ad_1915.png}
\caption{}
