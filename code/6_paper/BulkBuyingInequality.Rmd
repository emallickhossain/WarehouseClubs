...
Despite these savings, I find that households making less than \$25k are 6 percentage points less likely to make bulk purchases across a wide range of product categories, with especially large gaps in categories like paper towels (20 pp), toilet paper (20 pp), and diapers (13 pp). For storable items with relatively inelastic demand such as toilet paper or diapers, this presents a puzzle. Why are households not taking advantage of the bulk discounts available to reduce their expenditures? 
...
<!-- This paper combines reduced-form -->
...
# Stylized Facts
I document XX stylized facts to motivate my analysis:
...
2. Households making over \$100k are 6 percentage points more likely to use bulk discounts in addition to taking advantage of sales and coupons compared to households making under \$25k. Gaps are particularly large for essential storable items like toilet paper, diapers, and paper towels.
...
In order to compute the prevalence and magnitude of bulk discounts, I analyze purchases recorded in the Nielsen Consumer Panel data from 2004-2017. First, I compute the unit price for each item purchased. If multiple packages were purchased, I divide the total price paid by the number of packages to get the per-package price, which is then used to compute the unit price.^[Purchasing multiple packages generates linear pricing and without this correction would likely understate the magnitude of bulk discounts. To the extent that multiple packages were purchased because of a promotion (e.g. buy 2 get 1 free), maps that promotion into its equivalent per-package price.] Hence, all products within a product module are mapped into their corresponding unit price (e.g. cents per ounce for milk, cents per roll for toilet paper). 
...
<!-- This is the density plot separated by storability -->
<!-- \begin{figure} -->
<!-- \centering -->
<!-- \includegraphics{../5_figures/bulkDiscountAllProdsByStorability.png} -->
<!-- \caption{} -->
<!-- \label{fig:bulkDiscountAllProdsByStorability} -->
<!-- \end{figure} -->
...
<!-- \begin{equation} -->
<!-- log(UnitPrice)_{ismt} = \beta_1 Coupon_{ismt} + \beta_2 Sale_{ismt} + \beta_3 Generic_{ismt} + \sum_{q = 1}^4 \beta_4^q Quartile^q_{ismt} + \lambda_s + \lambda_m + \lambda_t + \epsilon_{ismt}, -->
<!-- \end{equation} -->

<!-- where $log(UnitPrice)$ is the log unit price of product $i$ purchased on shopping trip $s$ in market $m$ at time $t$. $Coupon$, $Sale$, and $Generic$ are all indicators for whether that item was purchased with a coupon, on sale, or was a generic brand, respectively. $Quartile^q$ is an indicator for whether the item's size is in the first, second, third, or fourth quartile of the size distribution of that product module. $\lambda$ are shopping trip, market, and year fixed effects. -->

<!-- In order to better understand the magnitude and prevalence of these discounts, I estimate this regression with different $\lambda_s$ fixed effects. I first estimate this regression with a retailer-module fixed effect which gives the average discount relative to the average unit price of that product module at a particular retailer (e.g. average per-sheet price of toilet paper at Target). Since brand can substantially affect average prices, I control for a retailer-brand fixed effect which then gives the average discount relative to the average unit price of a particular brand (e.g. average per-sheet price of Charmin toilet paper at Target). Since generics are a specific brand, it gets absorbed in the retailer-module-brand fixed effect. Finally, to pin down the savings from sales and coupons in particular, I control for a retailer-UPC fixed effect. This gives the average discount relative to the average unit price of a particular UPC (e.g. average per-sheet price of a 12-pack of Charmin UltraSoft toilet paper at Target). Overall, savings from each of these modes is substantial with coupons, generics, and bulk purchases saving the most money. The fact that generic brands and bulk purchases also provide the most savings is supported by @nevo2009. -->
