...
BulkShare_{it} = \sum_T \beta_1^t Yr_{it} + \gamma X_{it} + \lambda_i + \lambda_t + \epsilon_{it},
\end{equation}
...
Table \ref{tab:unitPriceLawMovers} reports the results of estimating Equation (\ref{eq:unitPriceMove}). Columns (1) and (2) show that a household's bulk buying is about one percentage point higher when they are in a state with unit price regulations. This specification implicitly assumes that the effect of moving to a state with unit price regulations will be the same as moving to a state without regulations (i.e. the effect is symmetric). Column (3) treats the different directions of moving differently and shows that moving to a state without unit price regulations significantly decreases bulk buying by 2.2 percentage points while moving to a state with regulations does not significantly change bulk buying. 
