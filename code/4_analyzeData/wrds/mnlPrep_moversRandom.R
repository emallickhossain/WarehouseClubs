...
# Focus on households that move to use random coefficients with panel data
library(data.table)
library(lubridate)
library(stringr)
library(stargazer)
...
################################################################################
# Step 1: Getting all mover households
################################################################################
panel <- fread("/scratch/upenn/hossaine/fullPanel.csv",
               select = c("household_code", "zip_code"))
panel[, "zips" := uniqueN(zip_code), by = household_code]
movers <- unique(panel[zips > 1]$household_code)

################################################################################
# Step 2: Getting all TP purchases for mover households
################################################################################
# Loading full trips data and restricting to trips made by mover households and
# stores with store codes
...
# About 12-50 shopping trips across 15k households
quantile(fullPurch[, uniqueN(trip_code_uc), by = household_code]$V1)
...
# Flagging any multi-UPC purchases (<4% of shopping trips or <4% of purchases)
...
# Flagging uncommon sizes (<15% of purchases)
...
fwrite(fullPurch, "/scratch/upenn/hossaine/fullTPPurchasesMovers.csv", nThread = threads)
...
# tar -xzvf tp.tar.gz
fullPurch <- fread("/scratch/upenn/hossaine/fullTPPurchasesMovers.csv",
                   nThread = threads)[drop == 0]

# Getting relevant store weeks
storeWeeks <- unique(fullPurch[, .(storeWeek = paste(store_code_uc, week_end, sep = "_"))])

# Getting all store assortments from relevant store-weeks
...
for (yr in yrs) {
  ...
  tp[, "storeWeek" := paste(store_code_uc, week_end, sep = "_")]
  tp <- merge(tp, storeWeeks, by = "storeWeek")
...
fwrite(fullTPSave, "/scratch/upenn/hossaine/fullTPAssortmentMovers.csv", nThread = threads)
...
fullPurch[, c("channel_type", "uncommonStore", "multiUPC", "uncommonBrand",
              ...
              "retailer_code") := NULL]
fullTP <- fread("/scratch/upenn/hossaine/fullTPAssortmentMovers.csv", nThread = threads)
fullChoice <- merge(fullPurch, fullTP, by = c("store_code_uc", "week_end"))
fullChoice[, "prmult" := NULL]
fwrite(fullChoice, "/scratch/upenn/hossaine/fullTPChoicesMovers.csv", nThread = threads)
...
fwrite(fullChoice, "/scratch/upenn/hossaine/fullChoiceMovers.csv", nThread = threads)

################################################################################
# Step 6: Identifying choices
################################################################################
fullChoice <- unique(fread("/scratch/upenn/hossaine/fullChoiceMovers.csv", nThread = threads))
...
# Matching brands (93% of branded trips are matched exactly, I drop the other 7%)
...
fwrite(fullChoiceFinal, "/scratch/upenn/hossaine/fullChoiceFinalMovers.csv",
       nThread = threads)
