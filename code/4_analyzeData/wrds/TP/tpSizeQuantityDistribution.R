...
ggplot(data = tp[brand_descr %in% topBrands & rolls <= 24],
       aes(x = rolls, y = days)) +
  geom_point(size = 0.2) +
  geom_hline(yintercept = sheetQtile25, linetype = "dashed") +
  geom_hline(yintercept = sheetQtile75, linetype = "dashed") +
  ...
  scale_color_grey()
ggsave(filename = "./figures/tpSizeQuantityDistributionAllProds.png",
       height = 4, width = 6)
...
ggplot(data = fullChoiceFinal[brand_descr %in% topBrands & rolls <= 24],
  ...
  theme(axis.title = element_text(),
        ...
        legend.position = "bottom") +
  scale_color_grey()
ggsave(filename = "./figures/tpSizeQuantityDistributionEstSample.png",
       height = 4, width = 6)
