# Getting the lowest possible price for product categories (milk, eggs, tampon, diapers)
...
getSales <- function(moduleCode, moduleName, fileNum) {
  path <- paste0("/home/mallick/Desktop/Nielsen/Data/Scanner/", moduleName, "/")
  ...
  for (yr in yrs) {
    ...
    sales[, "minUnitPrice" := min(unitPrice), by = .(store_code_uc, brand_code_uc, week_end)]
  ...
  fwrite(fullSales, paste0("/home/mallick/Downloads/full", moduleName, ".csv"),
         nThread = threads)
}
...
# cd Downloads/
# tar -czvf full [moduleName] .tar.gz full [moduleName].csv
# scp /home/mallick/Downloads/full [moduleName].tar.gz hossaine@wrds-cloud.wharton.upenn.edu:/scratch/upenn/hossaine
# cd /scratch/upenn/hossaine
# tar -xzvf full [moduleName].tar.gz
...
moduleCodes <- c(8444, 4100, 3625, 7270)
...
modKey <- data.table(modName = c("diaper", "eggs", "milk", "tampon"),
                     product_module_code = c(8444, 4100, 3625, 7270))
fullChoices <- NULL
for (mod in c("diaper", "eggs", "milk", "tampon")) {
...
fullChoices[, "savings" := (unitPriceChosen - minUnitPrice) / unitPriceChosen]
fullChoices[, "bigger" := (totalAmount < units)]
...
fullChoices <- merge(fullChoices, panel, by = c("household_code", "panel_year"))

# Computing missed savings by household
hhSavings <- fullChoices[bigger == TRUE,
