...
reg1 <- felm(log(totalAmount) ~ household_income + household_size + age |
               married + child,
             data = fullPurch, weights = fullPurch$projection_factor)
reg2 <- felm(log(totalAmount) ~ household_income + household_size + age |
               married + child + brand_code_uc + retailYear,
             data = fullPurch, weights = fullPurch$projection_factor)
reg3 <- felm(log(totalAmount) ~ household_income + household_size + age |
               married + child + brand_code_uc + brandRetailYear,
             data = fullPurch, weights = fullPurch$projection_factor)
reg4 <- felm(log(totalAmount) ~ household_income + household_size + age |
               married + child + brand_code_uc + zipYear,
             data = fullPurch, weights = fullPurch$projection_factor)
reg5 <- felm(log(totalAmount) ~ household_income + household_size + age |
               married + child + brand_code_uc + brandZipYear,
             data = fullPurch, weights = fullPurch$projection_factor)
stargazer(reg1, reg2, reg3, reg4, reg5, type = "text", omit.stat = c("f", "ser"))

# Bulk buying by sheets
reg1 <- felm(log(totalSheet) ~ household_income + household_size + age |
               married + child,
             data = fullPurch, weights = fullPurch$projection_factor)
reg2 <- felm(log(totalSheet) ~ household_income + household_size + age |
               married + child + brand_code_uc + retailYear,
             data = fullPurch, weights = fullPurch$projection_factor)
reg3 <- felm(log(totalSheet) ~ household_income + household_size + age |
               married + child + brand_code_uc + brandRetailYear,
             data = fullPurch, weights = fullPurch$projection_factor)
reg4 <- felm(log(totalSheet) ~ household_income + household_size + age |
               married + child + brand_code_uc + zipYear,
             data = fullPurch, weights = fullPurch$projection_factor)
reg5 <- felm(log(totalSheet) ~ household_income + household_size + age |
               married + child + brand_code_uc + brandZipYear,
             data = fullPurch, weights = fullPurch$projection_factor)
stargazer(reg1, reg2, reg3, reg4, reg5, type = "text", omit.stat = c("f", "ser"))
