...
getSales <- function(moduleCode, moduleName, groupNum) {
  ...
  for (yr in yrs) {
    print(yr)
    sales <- fread(paste0(path, yr, "/Movement_Files/", groupNum, "_", yr, "/", moduleCode, "_", yr, ".tsv"),
                   nThread = threads,
    ...
    # Getting min unit price and recording size of min unit price for each package size
...
moduleCodes <- c(8444, 7734, 4100, 3625, 7270)
...
# Combining with Homescan data by store, brand, and week
modKey <- data.table(modName = c("diaper", "paperTowel", "eggs", "milk", "tampon"),
                     product_module_code = c(8444, 7734, 4100, 3625, 7270))
fullChoices <- NULL
...
hhSavings <- bestPrice[savings >= 0,
                       ...
                       by = .(household_code, panel_year, totalAnnualSpending)]
...
reg1 <- felm(data = hhSavings[mod == "diaper"],
             weekSavings ~ household_income_coarse + age + nChildren +
               men + women + married + college | as.factor(dma_cd) + as.factor(panel_year))
reg2 <- felm(data = hhSavings[mod == "paperTowel"],
             weekSavings ~ household_income_coarse + age + nChildren +
               men + women + married + college | as.factor(dma_cd) + as.factor(panel_year))
...
reg4 <- felm(data = hhSavings[mod == "milk"],
             weekSavings ~ household_income_coarse + age + nChildren +
               men + women + married + college | as.factor(dma_cd) + as.factor(panel_year))
reg5 <- felm(data = hhSavings[mod == "tampon"],
             weekSavings ~ household_income_coarse + age + nChildren +
               men + women + married + college | as.factor(dma_cd) + as.factor(panel_year))
