# Generic HHI by channel and income
...
getData <- function(yr) {
  ...
  avgSpend <- fullData[, .(spend = weighted.mean(spend, w = projection_factor)),
                       by = .(panel_year, date, brand_code_uc, channel_type, household_income)]
...
hhi <- fullData[, .(hhi = sum(spendShare ^ 2)),
                by = .(panel_year, date, channel_type, household_income)]
setorder(hhi, channel_type, household_income, date)
hhi[, "rollMean" := rollmean(hhi, 4, fill = c(NA, NULL, NA), align = "right"),
    by = .(channel_type, household_income)]

getPlots <- function(storeType, legend) {
  p <- plot_ly(data = hhi, x = ~date, width = 1200, height = 800, showlegend = legend) %>%
    add_lines(data = hhi[channel_type == storeType & household_income == "<25k"],
    ...
    add_lines(data = hhi[channel_type == storeType & household_income == ">100k"],
    ...
    layout(xaxis = list(title = "Year", titlefont = list(size = 30),
           ...
           yaxis = list(title = "Generic Share", range = c(0, 0.25), dtick = 0.05,
...
chart <- subplot(charts, nrows = 2, shareX = TRUE, shareY = TRUE, margin = 0.03) %>%
  ...
  add_annotations("Dollar Store", x = 0.8, y = 0.2, xref = "paper",
                  yref = "paper", showarrow = FALSE, font = list(size = 30)) %>%
  layout(title = "Herfindahl Index by Channel and Income (2004-2016)",
...
export(chart, "./code/5_figures/genericHHIIncome.png")
