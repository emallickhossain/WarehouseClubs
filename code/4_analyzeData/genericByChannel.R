...
getData <- function(yr) {
  ...
  avgSpend <- fullData[, .(spend = weighted.mean(spend, w = projection_factor)),
                       by = .(panel_year, date, generic, channel_type)]
...
setorder(graphData, channel_type, date)
...
chart <- plot_ly(data = graphData, x = ~date, height = 800, width = 1200) %>%
  ...
  add_lines(data = rec, x = ~date, y = ~value * 1000,
            ...
            showlegend = F,
            hoverinfo = "none") %>%
  layout(title = "Generic Expenditures by Channel (2004-2016)",
